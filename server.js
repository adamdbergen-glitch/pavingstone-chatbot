...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // If the previous assistant message mentioned an estimate/ballpark/price/cost/quote
    // and the user replied with anything that is NOT clearly "no / not yet / later",
    // treat that as asking for a price (covers "that's it", "all good", etc.).
    if (
      lastMsg?.role === "user" &&
