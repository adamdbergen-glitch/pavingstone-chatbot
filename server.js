...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // 1) Square footage
    let sqft = 0;
    const dimMatch = lastUser.match(/(\d+)\s*x\s*(\d+)/); // e.g. "20x20"
    if (dimMatch) {
      const length = parseInt(dimMatch[1], 10);
      const width = parseInt(dimMatch[2], 10);
      sqft = length * width;
    } else {
      const sqMatch = lastUser.match(/(\d+)\s*(sqft|square feet|square foot|sq ft)/);
      if (sqMatch) {
    ...
    if (allUserText.includes("easy access") || allUserText.includes("good access")) {
      access_level = "easy";
    }
    if (
    ...
    if (allUserText.includes("winnipeg")) {
      ...
      city_town = "Selkirk";
      is_out_of_town = true;
    } else if (allUserText.includes("morden") || allUserText.includes("winkler")) {
    ...
    let userAskedPrice = /\b(price|cost|estimate|ballpark|quote|how much)\b/.test(lastUser);
    ...
    if (
      lastMsg?.role === "user" &&
      /^(yes|yep|yeah|sure|ok|okay|sounds good).*$/i.test(lastMsg.content || "") &&
  ...
  // NEW: describe whether the chosen stone is budget / midrange / premium
